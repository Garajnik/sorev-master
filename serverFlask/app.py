...
def get_local_ip_route():
    return jsonify({"local_ip": list_ipv4_addresses()}), 200
...
participants = {"redName": "Имя Красного Участника", "blueName": "Имя Синего Участника"}
...
def submit_judge_name():
    ...
    socketio.emit("update_judges", {"connected_judges": connected_judges})
...
def handle_button_click():
    ...
    # Логирование полученных данных
    print(
        f"Judge Name: {judge_name}, Button Index: {button_index}, Button Column: {button_column}, Button Row: {button_row}"
    )
